...
final class GitHubListOnSubscribe<T> implements OnSubscribe<T> {
  ...
  private static class BodySubscriber<HitHubPagingBody> extends Subscriber<Response<HitHubPagingBody>> {
    private final Subscriber<? super HitHubPagingBody> subscriber;
    /** Indicates whether a terminal event has been sent to {@link #subscriber}. */
    private boolean subscriberTerminated;

    BodySubscriber(Subscriber<? super HitHubPagingBody> subscriber) {
      super(subscriber);
      this.subscriber = subscriber;
    }

    @Override public void onNext(Response<HitHubPagingBody> response) {
      if (response.isSuccessful()) {
        GitHubPaging<?> paging;
        if (response.body() instanceof GitHubPaging){
          paging = (GitHubPaging<?>) response.body();
        }else {
          throw new IllegalArgumentException("response.boey type error:"+response.body().getClass());
        }
